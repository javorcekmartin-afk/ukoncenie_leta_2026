// v23 – kompaktnejšia a logicky zoradená tabuľka produktov + jednotky l / ks + vlastné poradie.
(function(){
  ...
  function actionCell(p){
    ...
    const downDisabled=idx<0||idx>=state.products.length-1?'disabled':'';
    return `<div class="product-actions">
      <button class="small order-btn" title="Posunúť vyššie" data-reorder="up" data-id="${p.id}" ${upDisabled}>↑</button>
      <button class="small order-btn" title="Posunúť nižšie" data-reorder="down" data-id="${p.id}" ${downDisabled}>↓</button>
      <button class="small danger" data-action="deleteProduct" data-id="${p.id}" title="Zmazať produkt">×</button>
    </div>`;
  }
  ...
  style.textContent=`
    ...
    #tab-products .product-actions{display:flex;gap:4px;align-items:center;white-space:nowrap}
    #tab-products .order-btn{min-width:32px;font-size:16px;line-height:1}
    #tab-products .order-btn:disabled{opacity:.3;cursor:not-allowed}
  `;
  ...
  // Ručné poradie produktov. Presúva sa objekt priamo v state.products, takže poradie zostáva uložené aj po obnovení stránky.
  document.addEventListener('click',e=>{
    const btn=e.target.closest('[data-reorder]');
    if(!btn||btn.disabled)return;
    const idx=state.products.findIndex(p=>p.id===btn.dataset.id);
    if(idx<0)return;
    const next=btn.dataset.reorder==='up'?idx-1:idx+1;
