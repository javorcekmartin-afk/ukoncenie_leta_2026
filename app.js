const KEY="kostoliste_stanok_v13";
const OLD_KEYS=["kostoliste_stanok_v12","kostoliste_stanok_v11","kostoliste_stanok_v10","kostoliste_stanok_v9","kostoliste_stanok_v8","kostoliste_stanok_v3"];
...
function migrateOld(old){
  ...
  s.products=(old.products||[]).map(p=>{
    const saleAmount=n(p.saleAmount??p.unitLiters)||.5,packageAmount=n(p.packageAmount??p.packageLiters)||1;
    const suggested=packageAmount>0&&saleAmount>0?Math.ceil(n(p.plannedQty??p.estimatedUnits)*saleAmount/packageAmount):0;
    return {id:p.id||uid(),name:p.name||"Produkt",category:p.category||"Iné",mode:p.mode||"simple",supplier:p.supplier||"",unit:p.unit||"L",saleAmount,packageAmount,packagePrice:n(p.packagePrice),salePrice:n(p.salePrice),plannedQty:n(p.plannedQty??p.estimatedUnits),plannedPackages:n(p.plannedPackages)||suggested,components:Array.isArray(p.components)?p.components:(Array.isArray(p.recipe)?p.recipe.map(r=>({itemId:r.ingredientId,amount:n(r.amount)})):[])};
  });
  const oldInv=old.inventory&&typeof old.inventory==="object"?old.inventory:{};
  Object.entries(oldInv).forEach(([key,v])=>{
    const actual = String(v.usedPackages??"").trim()!=="" ? v.usedPackages : (String(v.actualPackages??"").trim()!=="" ? v.actualPackages : "");
    s.inventory[key]={actualPackages:actual,actualUnitPrice:v.actualUnitPrice??"",supplierOverride:v.supplierOverride??"",note:v.note??""};
  });
  if(Array.isArray(old.manualInventory))s.manualInventory=old.manualInventory.map(x=>({
    id:x.id||uid(),name:x.name||"Manuálna položka",supplier:x.supplier||"",packageLabel:x.packageLabel||"1 balenie",
    packagePrice:n(x.packagePrice),plannedPackages:n(x.plannedPackages),
    actualPackages:String(x.usedPackages??"").trim()!==""?x.usedPackages:(x.actualPackages??""),
    actualUnitPrice:x.actualUnitPrice??"",note:x.note||""
  }));
  s.supplementalCosts=(old.supplementalCosts||old.fixedCosts||[]).map(c=>({id:c.id||uid(),name:c.name||"Náklad",category:c.category||"Ostatné",planAmount:n(c.planAmount??c.amount),qty:n(c.qty||1),unitPrice:n(c.unitPrice??c.amount),refund:n(c.refund),note:c.note||""}));
  return s;
}
function normalize(s){
  ...
  s.manualInventory=Array.isArray(s.manualInventory)?s.manualInventory:[];
  s.supplementalCosts=Array.isArray(s.supplementalCosts)?s.supplementalCosts:[];
  s.products=s.products.map(p=>({...p,supplier:p.supplier||""}));
  s.items=s.items.map(i=>({...i,supplier:i.supplier||"",plannedPackages:n(i.plannedPackages)}));
  s.version=13;return s;
}
function load(){
  try{
    const raw=localStorage.getItem(KEY);if(raw)return normalize(JSON.parse(raw));
    for(const k of OLD_KEYS){const old=localStorage.getItem(k);if(old){const m=normalize(migrateOld(JSON.parse(old)));localStorage.setItem(KEY,JSON.stringify(m));return m}}
  }catch(e){}
  return baseState();
}
...
function productCalc(p){
  ...
  const suggested=p.mode==="simple"&&n(p.packageAmount)>0?Math.ceil(n(p.plannedQty)*n(p.saleAmount)/n(p.packageAmount)):0;
  const planned=n(p.plannedPackages)||suggested;
  return {base:b,components:comp,cost,profit,markup:cost>0?profit/cost*100:0,recommended:roundUp(cost*(1+n(state.meta.targetMarkup)/100),.1),servingsPerPackage:servings,suggestedPackages:suggested,plannedPackages:planned,capacity:servings*planned};
}
...
function sourceInventoryRows(){
  ...
  state.products.forEach(p=>{
    if(p.mode==="simple"&&n(p.packageAmount)>0&&n(p.saleAmount)>0){
      const c=productCalc(p);rows.push({key:"product:"+p.id,source:"product",sourceId:p.id,name:p.name,supplier:p.supplier||"Bez obchodu",packageLabel:`${num.format(n(p.packageAmount))} ${p.unit||"L"}`,packagePrice:n(p.packagePrice),minimum:c.suggestedPackages,planned:c.plannedPackages,manual:false});
    }
  });
  state.items.forEach(i=>{
    const need=itemDemand(i.id);if(need>0)rows.push({key:"item:"+i.id,source:"item",sourceId:i.id,name:i.name,supplier:i.supplier||"Bez obchodu",packageLabel:`${num.format(n(i.packageAmount))} ${i.unit||"L"}`,packagePrice:n(i.packagePrice),minimum:itemSuggestedPackages(i),planned:itemPlannedPackages(i),manual:false});
  });
  return rows;
}
function inventoryRows(){
  return [...sourceInventoryRows(),...state.manualInventory.map(m=>({key:"manual:"+m.id,source:"manual",sourceId:m.id,name:m.name||"Manuálna položka",supplier:m.supplier||"Bez obchodu",packageLabel:m.packageLabel||"1 balenie",packagePrice:n(m.packagePrice),minimum:0,planned:n(m.plannedPackages),manual:true}))];
}
function invState(row){
  if(row.manual)return state.manualInventory.find(x=>x.id===row.sourceId);
  state.inventory[row.key]=state.inventory[row.key]||{actualPackages:"",actualUnitPrice:"",supplierOverride:"",note:""};
  return state.inventory[row.key];
}
...
function invCalc(row){
  const v=invState(row);
  const actual=String(v.actualPackages??"").trim()===""?0:n(v.actualPackages);
  const unitPrice=String(v.actualUnitPrice??"").trim()===""?n(row.packagePrice):n(v.actualUnitPrice);
  return {actual,unitPrice,realCost:actual*unitPrice,planPurchase:n(row.planned)*n(row.packagePrice)};
}
function suppCalc(c){const explicit=n(c.unitPrice)>0||n(c.refund)>0;return {actual:explicit?Math.max(0,n(c.qty||1)*n(c.unitPrice)-n(c.refund)):n(c.planAmount)}}
function hasInventory(){return inventoryRows().some(r=>String(invState(r).actualPackages??"").trim()!=="")}
function revenueCalc(){
  const cashFilled=String(state.meta.cashRevenue??"").trim()!=="",terminalFilled=String(state.meta.terminalRevenue??"").trim()!=="",floatFilled=String(state.meta.cashFloat??"").trim()!=="";
  const filled=cashFilled||terminalFilled||floatFilled,cash=n(state.meta.cashRevenue),terminal=n(state.meta.terminalRevenue),float=n(state.meta.cashFloat);
  return {filled,cash,terminal,float,total:filled?(-float+cash+terminal):null};
}
function totals(){
  let planRevenue=0;state.products.forEach(p=>planRevenue+=n(p.salePrice)*n(p.plannedQty));
  let planPurchase=0,actualPurchase=0;inventoryRows().forEach(r=>{const c=invCalc(r);planPurchase+=c.planPurchase;actualPurchase+=c.realCost});
  let suppPlan=0,suppActual=0;state.supplementalCosts.forEach(c=>{suppPlan+=n(c.planAmount);suppActual+=suppCalc(c).actual});
  const rev=revenueCalc(),planResult=planRevenue-planPurchase-suppPlan,actualResult=rev.filled?rev.total-actualPurchase-suppActual:null,cover=hasInventory()?actualPurchase+suppActual:planPurchase+suppPlan;
  return {planRevenue,planPurchase,suppPlan,planResult,breakEven:cover,actualRevenue:rev.total,actualPurchase,suppActual,actualResult,revenueFilled:rev.filled,rev};
}
function shopSummary(){
  const groups={};inventoryRows().forEach(r=>{const supplier=rowSupplier(r),g=groups[supplier]||(groups[supplier]={supplier,rows:[],plan:0,actual:0}),c=invCalc(r);g.rows.push({name:r.name,planned:r.planned,planValue:c.planPurchase,actual:c.actual,actualValue:c.realCost});g.plan+=c.planPurchase;g.actual+=c.realCost});
...
function renderCosts(){costBody.innerHTML=state.supplementalCosts.map(c=>`<tr><td>${field("cost",c.id,"name",c.name,"text","name")}</td><td>${costCategory(c.id,c.category)}</td><td>${moneyField("cost",c.id,"planAmount",c.planAmount)}</td><td>${field("cost",c.id,"qty",c.qty,"num")}</td><td>${moneyField("cost",c.id,"unitPrice",c.unitPrice)}</td><td>${moneyField("cost",c.id,"refund",c.refund)}</td><td class="calc">${eur.format(suppCalc(c).actual)}</td><td>${field("cost",c.id,"note",c.note||"")}</td><td><button class="small danger" data-action="deleteCost" data-id="${c.id}">×</button></td></tr>`).join("");costEmpty.hidden=state.supplementalCosts.length>0}
...
function updateSummary(){
  const t=totals();setMoney("planRevenue",t.planRevenue);setMoney("planPurchase",t.planPurchase);setMoney("planResult",t.planResult);setMoney("breakEven",t.breakEven);setMoney("actualRevenue",t.actualRevenue,t.revenueFilled);setMoney("actualPurchase",t.actualPurchase);setMoney("actualSupplemental",t.suppActual);setMoney("actualResult",t.actualResult,t.revenueFilled);
  setMoney("rPlanRevenue",t.planRevenue);setMoney("rPlanPurchase",t.planPurchase);setMoney("rSuppPlan",t.suppPlan);setMoney("rPlanResult",t.planResult);setMoney("rBreakEven",t.breakEven);setMoney("rCashRevenue",t.rev.cash,String(state.meta.cashRevenue??"").trim()!=="");setMoney("rTerminalRevenue",t.rev.terminal,String(state.meta.terminalRevenue??"").trim()!=="");setMoney("rCashFloat",-t.rev.float,String(state.meta.cashFloat??"").trim()!=="");setMoney("rActualRevenue",t.actualRevenue,t.revenueFilled);setMoney("rActualPurchase",t.actualPurchase);setMoney("rSuppActual",t.suppActual);setMoney("rActualResult",t.actualResult,t.revenueFilled);
}
...
const numericFields=new Set(["saleAmount","packageAmount","packagePrice","salePrice","plannedQty","plannedPackages","actualPackages","actualUnitPrice","planAmount","qty","unitPrice","refund"]);
...
exportJson.onclick=()=>download("stanok-data-v13.json",JSON.stringify({...state,exportedAt:new Date().toISOString()},null,2),"application/json");
...
exportCsv.onclick=()=>{const t=totals(),rows=[["AKCIA",state.meta.eventName],["Pokladňa",t.rev.cash],["Terminál",t.rev.terminal],["- Vklad",-t.rev.float],["Skutočná tržba",t.rev.total??""],[],["INVENTÚRA","Položka","Obchod","Plán balení","Skutočne minuté balenia","Cena/balenie","Reálny náklad"]];inventoryRows().forEach(r=>{const c=invCalc(r);rows.push(["INVENTÚRA",r.name,rowSupplier(r),r.planned,c.actual,c.unitPrice,c.realCost])});rows.push([]);rows.push(["SÚHRN","Plán nákup",t.planPurchase,"Reálny náklad tovaru",t.actualPurchase,"Doplnkové náklady",t.suppActual,"Skutočný výsledok",t.actualResult??""]);const q=v=>`"${String(v??"").replaceAll('"','""')}"`;download("stanok-prehlad-v13.csv","\ufeff"+rows.map(r=>r.map(q).join(";")).join("\n"),"text/csv;charset=utf-8")};
